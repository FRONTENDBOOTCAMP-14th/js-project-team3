...
export function formatMatchDataForOtherInfo(props) {
  console.log("formatMatchDataForOtherInfo props", props);
  const { userBasicInfo, userTierInfo, matchList } = props;
  const { match} = matchList


  // 필요한 데이터가 하나라도 없으면 빈 배열을 반환하여 에러 방지
  if (!userBasicInfo || !userTierInfo || !matchList) {
    return [];
  }

  // 1. match_type을 기준으로 전적 데이터를 집계합니다.
  const statsByMatchType = match.reduce((acc, match) => {
