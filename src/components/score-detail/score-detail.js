const VITE_NEXON_OPEN_API_KEY = "test_eabdd18d9c77e72c6357c53586911fb20d9c3a31ca6428e4e34ee92bdf5812dbefe8d04e6d233bd35cf2fabdeb93fb0d"
...
function renderMatchItem(matchInfo) {
  ...
  const matchResultText = RESULT_KEY_VALUE[matchInfo.match_result] || RESULT_KEY_VALUE.DEFAULT
  const matchDateText = matchInfo.date_match ? getTimeAgo(matchInfo.date_match) : '-'
  ...
  const headshotCount = matchInfo.headshot !== undefined ? matchInfo.headshot : 'N/A'
  ...
  li.innerHTML = `
    <section class="match-preview-section">
      <div class="match-result-box ${matchResultText === '승리' ? 'win' : (matchResultText === '패배' ? 'lose' : 'draw')}">
      </div>

      <section class="match-padding-section" data-match-id="${matchInfo['match_id']}">
        <div class="match-date-box">
          <p class="match-result-text ${matchResultText === '승리' ? 'win' : (matchResultText === '패배' ? 'lose' : 'draw')}">${matchResultText}</p>
          <p class="match-date-text">${matchDateText}</p>
        </div>

        <div class="match-type-box match-type-box-web">
        ...
        <section class="match-stats-section grid-full-width-section">
          ...
          <div class="match-stats-box">
            ...
            <p class="match-stats-value">${kill}</p>
          </div>
          ...
          <div class="match-stats-box">
            ...
            <p class="match-stats-value">${death}</p>
          </div>
          ...
          <div class="match-stats-box">
            ...
            <p class="match-stats-value">${assist}</p>
          </div>
        </section>
        <button class="btn-match-detail grid-full-width-section" type="button" style="color:#fff">
          <!--
    ...
    <section id="section_${matchInfo['match_id']}" class="user-match-detail-wrapper" style="display:none">
      <section class="match-header-section">
        <p class="match-result-text ${matchResultText === '승리'? 'win' : 'lose'}">${matchResultText}</p>
        <p class="match-type-text map-name"></p>
        <p class="match-date-text">${convertToKoreanFormat(matchInfo['date_match'])}</p>
      </section>

      <section data-v-b1cb458b="" class="match-detail-section">
        <section data-v-b1cb458b="" class="match-team-section win">
            <section data-v-b1cb458b="" class="match-team-header-section win">
                <p data-v-b1cb458b="" class="match-header-text win">승리</p>
            </section>
            <section class="match-team-body-section win-section">
            </section>
        </section>
        <section data-v-b1cb458b="" class="match-team-section lose">
            <section data-v-b1cb458b="" class="match-team-header-section lose">
                <p data-v-b1cb458b="" class="match-header-text lose">패배</p>
            </section>
...
const init = () => {
  ...
  buttonUserInfo.addEventListener('click', () => {
    ...
    fetch (ouidcallUrl, {
      ...
      headers: {
        ...
        "x-nxopen-api-key": VITE_NEXON_OPEN_API_KEY,
      },
    })
  ...
  if (matchTypeUl) {
    Array.from(matchTypeUl.querySelectorAll(".match-type-item button")).forEach((button) => {
      ...
      button.addEventListener('click', () => {
        ...
        matchTypeUl.querySelectorAll(".btn-match-type").forEach(btn => btn.classList.remove('active'))
        button.classList.add('active')

        matchHistoryListUl.innerHTML = ''
        informationArea.innerHTML = '<p class="match-loading">매치 정보를 로딩 중입니다...</p>'
        ...
        .then (matchdata => {
          ...
          if (!matchdata.match || matchdata.match.length === 0) {
            matchHistoryListUl.innerHTML = '<li class="match-history-item"><p>해당 조건의 매치 결과가 없습니다.</p></li>'
            return
          }

          
          matchHistoryListUl.innerHTML = ''
          const detailPromises = matchdata.match.map(item => {

              //const li = document.createElement('li')
              //li.classList.add('match-history-item')

              const matchLi = renderMatchItem(item)
              // matchLi.classList.add('match-history-item')
              matchHistoryListUl.appendChild(matchLi)
          })

  

          Array.from(document.querySelectorAll('.btn-match-detail')).forEach((element)=>{
...
function renderMatchDetailTable(matchDetailInfo) {

}


const callDetailFatch = (matchId)=> {
    const matchDetailCallUrl = `${DOMAIN}/match-detail?match_id=${matchId}`
    ...
    .then (detail => {
      ...
      // 다른거 클릭했을때
        ...
        Array.from(document.querySelectorAll('.user-match-detail-wrapper')).forEach((element)=>{
        ...
        // 맵이름 찾아주기
        sectionElement.querySelector('.map-name').innerHTML = detail['match_map'];
        ...
        detail['match_detail'].forEach((item)=>{
...
function getDetailList(item){
  ...
  result.innerHTML  = `
        <li data-v-2e9d49a7="" class="match-team-item">
          <img data-v-2e9d49a7="" src="${getRankIcon(item['season_grade'])}" alt="계급" class="img-grade">
            <p data-v-2e9d49a7="" class="match-team-text"><a href="https://ezscope.gg/match/${item['user_name']}" class="btn-search-player" target="_blank">${item['user_name']}</a></p>
        </li>
        <li data-v-2e9d49a7="" class="match-team-item">
            <p data-v-2e9d49a7="" class="match-team-text rate-orange">${((item.kill / (item.kill + item.death)) * 100).toFixed(2)} <span data-v-2e9d49a7="" class="match-team-unit-text">%</span></p>
        </li>
        <li data-v-2e9d49a7="" class="match-team-item">
            <p data-v-2e9d49a7="" class="match-team-text">${item.kill} / <span data-v-2e9d49a7="" class="match-team-death-text">${item.death}</span> / ${item.assist}</p>
        </li>
        <li data-v-2e9d49a7="" class="match-team-item match-team-item-web">
            <p data-v-2e9d49a7="" class="match-team-text">${item.headshot}</p>
        </li>
        <li data-v-2e9d49a7="" class="match-team-item match-team-item-web">
            <p data-v-2e9d49a7="" class="match-team-text">${item.damage.toLocaleString('ko-KR')}</p>
        </li>`
  return  result;
}
...
function getDetailHeader (){
  return `
    <ul data-v-2e9d49a7="" class="match-team-label-list">
          <li data-v-2e9d49a7="" class="match-team-label-item">
              <p data-v-2e9d49a7="" class="match-team-label-text">플레이어</p>
          </li>
          <li class="match-team-label-item" data-v-2e9d49a7="">
              <p class="match-team-label-text" data-v-2e9d49a7="">킬뎃</p>
          </li>
          <li class="match-team-label-item" data-v-2e9d49a7="">
              <p class="match-team-label-text" data-v-2e9d49a7="">KDA</p>
          </li>
          <li class="match-team-label-item match-team-item-web" data-v-2e9d49a7="">
              <p class="match-team-label-text" data-v-2e9d49a7="">헤드샷</p>
          </li>
          <li class="match-team-label-item match-team-item-web" data-v-2e9d49a7="">
              <p class="match-team-label-text" data-v-2e9d49a7="">딜량</p>
          </li>
      </ul>`
}
...
addEventListener("DOMContentLoaded",init)
  ...
  // 매치목록 및 상세정보 가져오기
  // if (matchType) {
  //   Array.from(matchType.querySelectorAll("li")).forEach((li) => {
  //     area.innerHTML = '<p>매치 정보를 로딩 중입니다.</p>'
  ...
  //     .then (response => response.json())
  //     .then (matchdata => {
  //         matchdata.match.forEach(match => {
  //         const matchId = match.match_id
  //         const params = new URLSearchParams()
  //         params.set('match_id', matchId)
  //         const matchDetailListCallUrl = `${DOMAIN}/match_detail?match_id=${matchId}`

  //         fetch (matchDetailListCallUrl, {
  //           method: "GET",
  //           headers: {
  //             accept: "application/json",
  //             "x-nxopen-api-key": VITE_NEXON_OPEN_API_KEY,
  ...
  //           // 매치 목록을 가져와서 뿌려주기
  //           area.innerHTML = '<p></p>'
  //           const resultListP = area.querySelector('p')
