exports.handler = async function(event, context) {
    ...
    try {
        // 파라미터 추출
        let ouid, match_mode, match_type;
        if (event.httpMethod === "POST") {
            const body = JSON.parse(event.body);
            ouid = body.ouid;
            match_mode = body.match_mode || "폭파미션";
            match_type = body.match_type || "일반";
        } else {
            ouid = event.queryStringParameters?.ouid;
            match_mode = event.queryStringParameters?.match_mode || "폭파미션";
            match_type = event.queryStringParameters?.match_type || "일반";
        }
        ...
        // 서든어택 API 호출
        const response = await fetch(`https://open.api.nexon.com/suddenattack/v1/match?ouid=${ouid}&match_mode=${encodeURIComponent(match_mode)}&match_type=${encodeURIComponent(match_type)}`, {
            method: "GET",
            headers: {
