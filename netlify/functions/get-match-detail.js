    ...
    try {
        ...
        if (!match_id) {
            return {
                statusCode: 400,
                headers: headers,
                body: JSON.stringify({ error: "match_id parameter is required" })
            };
        }
        ...
        // 서든어택 API 호출
        const response = await fetch(`https://open.api.nexon.com/suddenattack/v1/match-detail?match_id=${match_id}`, {
            method: "GET",
            headers: {
